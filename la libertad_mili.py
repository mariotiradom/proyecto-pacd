...
# ---------------------------------------------------------------
# Completar y filtrar los datos para La Libertad
# ---------------------------------------------------------------
...
# c. Filtrar los registros solo para "LA LIBERTAD"
...
df_lalibertad=df_lalibertad.loc[df_lalibertad['VEHÍCULO'] != '¿POSTE?']
df_lalibertad = df_lalibertad.loc[df_lalibertad['CLASE SINIESTRO'] != 'CAÍDA DE PASAJERO']
df_lalibertad = df_lalibertad.loc[df_lalibertad['CLASE SINIESTRO'] != 'VOLCADURA']
reemplazos1 = {
...
df_lalibertad['VEHÍCULO'] = df_lalibertad['VEHÍCULO'].replace(reemplazos1)
...
df_lalibertad['CLASE SINIESTRO'] = df_lalibertad['CLASE SINIESTRO'].replace(reemplazos2)

# Mostrar los datos filtrados
print(df_lalibertad.head())


##MAPA DE CALOR

g1 = df_lalibertad.groupby(['VEHÍCULO', 'CLASE SINIESTRO']).size().reset_index(name='contador')
...
fig1 = px.density_heatmap(g1, x='CLASE SINIESTRO', y='VEHÍCULO', z='contador', 
                         ...
                         title='Tipo de Siniestro vs Tipo de Vehículo en La Libertad',
                         color_continuous_scale='Blues',  
                         text_auto=True,                   
                         range_color=[0, g1['contador'].max()])  

fig1.show()
fig1.write_html('mi_grafico1.html')
webbrowser.open('mi_grafico1.html')
...
# === GRÁFICO DE LÍNEAS INTERACTIVO CON PROYECCIONES PARA LA LIBERTAD ===
def grafico_lineas_la_libertad(df, nombre_html):
    df = df[df['DEPARTAMENTO'].str.upper() == "LA LIBERTAD"]
    ...
    fig = px.line(df_proy, x='Año', y='Cantidad', color='Tipo',
                  title="Proyección: Tipos de siniestro más frecuentes en La Libertad de Motocicletas",
                  markers=True,
...
# Llamar a la función para generar el gráfico de proyección para "La Libertad"
grafico_lineas_la_libertad(vehiculos_siniestros, "proyeccion_lalibertad.html")
webbrowser.open('proyeccion_lalibertad.html')
