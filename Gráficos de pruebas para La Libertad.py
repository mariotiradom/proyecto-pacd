# Gráficos de pruebas para La Libertad
...
#%% Completar y filtrar para solo datos de La Libertad
...
# c. Filtro para obtener solo registros de La Libertad

filtro_lalibertad=df_filtro_siniestros_excluidos["DEPARTAMENTO"]=="LA LIBERTAD"
df_lalibertad=df_filtro_siniestros_excluidos.loc[filtro_lalibertad]


#%% Prueba de independencia para tipo de vía y clase de siniestro en La Libertad

# a. Agrupar variables para hacer la prueba debido a que las frecuencias son menores a 5 en estas categorías

df_lalibertad['clase_siniestro_mod'] = df_lalibertad['CLASE SINIESTRO'].replace(to_replace=['CAÍDA DE PASAJERO','CHOQUE FUGA','VOLCADURA','ATROPELLO FUGA'],value=4*["OTROS"])

df_lalibertad['tipo_via_mod'] = df_lalibertad['TIPO DE VÍA'].replace(to_replace=["JIRÓN",'PASAJE','OTRO'],value=3*['OTRAS VÍAS'])


# b. Creación de tabla de doble entrada de clase de siniestro modificado y tipo de via modificado

tabla_libertad = pd.crosstab(df_lalibertad['clase_siniestro_mod'], df_lalibertad['tipo_via_mod'])
print("-----Prueba de independencia-----\nTipo de Siniestros vs. Tipo de Vía")
print("\n")
print(tabla_libertad)
print("\n")

# c. Prueba de independencia
chi2, p, dof, expected_libertad = chi2_contingency(tabla_libertad)
...
expected_df_libertad = pd.DataFrame(expected_libertad, index=tabla_libertad.index, columns=tabla_libertad.columns)

# e. Restar lo observado y lo esperado para obtener el exceso o déficit observado frente al esperado
diferencia_libertad= tabla_libertad - expected_df_libertad

# f. Mostrar la diferencia
print("-----Tabla del exceso o déficit de siniestros-----")
print(diferencia_libertad)
print("\n")
...
grafico_mapa_calor_tabla_libertad=mapa_calor(tabla_libertad, 'YlGnBu', "Mapa de calor\nTipo de Vía vs Clase de Siniestro", 'Tipo de Vía', 'Tipo de Siniestro',0,100)

grafico_mapa_calor_interactivo_libertad=grafico_mapa_calor_interactivo(tabla_libertad, "Mapa de calor\nTipo de Vía vs Clase de Siniestro", 'Tipo de Vía', 'Tipo de Siniestro', "Grafico_mapa_calor_libertad.html")


# Mapa de calor de tabla de diferencia de frecuencias

grafico_mapa_calor_diferencia_libertad=mapa_calor(diferencia_libertad, "coolwarm", 'Mapa de calor del exceso o déficit de siniestros observados\nrespecto a los esperados según la prueba de independencia\nentre tipo de vía y clase de siniestro', 'Tipo de Vía', 'Tipo de Siniestro',-1,15)
grafico_mapa_interactivo_diferencia_libertad=grafico_mapa_calor_interactivo(diferencia_libertad, 'Mapa de calor del exceso o déficit de siniestros observados\nrespecto a los esperados según la prueba de independencia\nentre tipo de vía y clase de siniestro', 'Tipo de Vía', 'Tipo de Siniestro',"Grafico_mapa_diferencias_libertad.html")
...
combinacion_01 = df_lalibertad[(df_lalibertad["clase_siniestro_mod"] == "CHOQUE") &
    (df_lalibertad["tipo_via_mod"] == "CARRETERA")].groupby("AÑO").size()

# b. Filtrar despistes en carreteras
combinacion_02 = df_lalibertad[(df_lalibertad["clase_siniestro_mod"] == "DESPISTE") &
    (df_lalibertad["tipo_via_mod"] == "CARRETERA")].groupby("AÑO").size()
...
# a. Gráficos
grafico_lineas_combinaciones=grafico_lineas(eje_y_combinacion_01, eje_y_combinacion_02, "Evolución de siniestros críticos en La Libertad", 'Choques en Carretera', 'Despistes en Carretera', "Año", "Cantidad de siniestros")
grafico_lineas_interactivo_combinaciones=grafico_lineas_interactivo(eje_y_combinacion_01, eje_y_combinacion_02, "Evolución de siniestros críticos en La Libertad", 'Choques en Carretera', 'Despistes en Carretera', "Año", "Cantidad de siniestros", "Gráfico líneas La Libertad.html")
